    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-16 text-slate-400"><Bell className="w-12 h-12 mx-auto mb-3 text-slate-200" /><p className="font-semibold">No notifications yet</p><p className="text-sm mt-1">Order updates and alerts will appear here</p></div>
        ) : (
          <div className="divide-y divide-slate-100">
            {notifications.map(n => (
              <button key={n.id} onClick={() => !n.isRead && markOneRead(n.id)} className={`w-full text-left p-4 flex items-start gap-3 hover:bg-slate-50 transition-colors ${!n.isRead ? 'bg-orange-50/40' : ''}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-sm font-semibold">{n.title}</p><p className="text-sm text-slate-500 mt-0.5">{n.body}</p><p className="text-xs text-slate-400 mt-1">{formatRelativeTime(n.createdAt)}</p></div>
